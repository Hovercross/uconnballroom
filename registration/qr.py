...
qrCodeLevels = [0, 10, 20, 35, 50, 64, 84, 93, 122, 143, 174]
...
def getQRCodeData(data):
    level = 1
    while len(data)+4 > qrCodeLevels[level]:
        level += 1

    qrCode = QRCode(level, QRErrorCorrectLevel.H)
    qrCode.addData(str(data))
    qrCode.make()

    return qrCode.modules
    
def drawQRCode(canvas, data, innerSize, marginSize):
    matrix = getQRCodeData(data)

    pointsAcross = len(matrix[0])
    pointSize = innerSize / pointsAcross

    for i in range(pointsAcross):
        yBottom = (pointsAcross - i - 1) * pointSize + marginSize

        for j in range(pointsAcross):
            xLeft = (j-1) * pointSize + marginSize
            ...
            if on:
                ...
                if right:
                    drawn = True
                    canvas.rect(xLeft, yBottom, pointSize*2, pointSize, fill=1, stroke=0)
                    
                if bottom:
                    drawn = True
                    canvas.rect(xLeft, yBottom-pointSize, pointSize, pointSize*2, fill=1, stroke=0)

                if not drawn:
                    canvas.rect(xLeft, yBottom, pointSize, pointSize, fill=1, stroke=0)
                    
class QRCodeFlowable(Flowable):
    def __init__(self, data, size, margin = .125*inch):
        self.size = size
        self.data = data
        self.margin = margin
        
    def wrap(self, *args): 
        return (self.size + self.margin * 2, self.size + self.margin*2)
        
    def draw(self): 
        canvas = self.canv
        canvas.setFillColor(colors.black)
        drawQRCode(canvas, self.data, self.size, self.margin)
